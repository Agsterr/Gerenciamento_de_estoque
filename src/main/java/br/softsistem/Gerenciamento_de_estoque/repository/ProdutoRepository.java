...
public interface ProdutoRepository extends JpaRepository<Produto, Long> {
    ...
    // Busca um produto pelo ID e pela organização
    Produto findByIdAndOrgId(Long id, Long orgId);  // Novo método
    ...
    @Query("SELECT SUM(p.quantidadeEntrada) FROM Produto p WHERE FUNCTION('YEAR', p.dataEntrada) = :ano AND p.org.id = :orgId")
    BigDecimal totalEntradasPorAno(Long orgId, int ano);

    // Total de entradas de um produto por mês e organização
    @Query("SELECT SUM(p.quantidadeEntrada) FROM Produto p WHERE FUNCTION('YEAR', p.dataEntrada) = :ano AND FUNCTION('MONTH', p.dataEntrada) = :mes AND p.org.id = :orgId")
    BigDecimal totalEntradasPorMes(Long orgId, int ano, int mes);

    // Total de entradas de um produto por semana e organização
    @Query("SELECT SUM(p.quantidadeEntrada) FROM Produto p WHERE p.dataEntrada BETWEEN :inicioSemana AND :fimSemana AND p.org.id = :orgId")
    ...
    @Query("SELECT SUM(p.quantidadeEntrada) FROM Produto p WHERE p.dataEntrada = :dia AND p.org.id = :orgId")
    BigDecimal totalEntradasPorDia(Long orgId, LocalDate dia);

    // Total de saídas de um produto por ano e organização
    @Query("SELECT SUM(p.quantidadeSaida) FROM Produto p WHERE FUNCTION('YEAR', p.dataSaida) = :ano AND p.org.id = :orgId")
    BigDecimal totalSaidasPorAno(Long orgId, int ano);

    // Total de saídas de um produto por mês e organização
    @Query("SELECT SUM(p.quantidadeSaida) FROM Produto p WHERE FUNCTION('YEAR', p.dataSaida) = :ano AND FUNCTION('MONTH', p.dataSaida) = :mes AND p.org.id = :orgId")
    BigDecimal totalSaidasPorMes(Long orgId, int ano, int mes);

    // Total de saídas de um produto por semana e organização
    @Query("SELECT SUM(p.quantidadeSaida) FROM Produto p WHERE p.dataSaida BETWEEN :inicioSemana AND :fimSemana AND p.org.id = :orgId")
    ...
    @Query("SELECT SUM(p.quantidadeSaida) FROM Produto p WHERE p.dataSaida = :dia AND p.org.id = :orgId")
